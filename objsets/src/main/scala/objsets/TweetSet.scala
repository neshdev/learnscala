...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def descend(ts: TweetSet, acc: TweetList): TweetList = {
      if (ts.isEmpty) acc
      else {
        val mrt = ts.mostRetweeted
        val removedTs = ts.remove(mrt)
        descend(removedTs, new Cons(mrt, acc))
      }
    }
    descend(this, Nil).rev
  }

  def mostRetweetedAcc(ts: TweetSet, acc: Tweet): Tweet = {
    val filtered = ts.filter(t => acc.retweets < t.retweets)
    if (filtered.isEmpty) acc
    else {
      mostRetweetedAcc(filtered, ts.tweet)
    }
  }
...
trait TweetList {
  ...
  def rev: TweetList = {
    def revRec(ts: TweetList, acc: TweetList): TweetList = {
      if (ts.isEmpty) acc
      else revRec(ts.tail, new Cons(ts.head, acc))
    }
    revRec(this, Nil)
  }
}
...
object GoogleVsApple {
  ...
  def loopAcc(list: List[String], acc: TweetSet): TweetSet = {
    if (list.isEmpty) acc
    else {
      val alltweets = TweetReader.allTweets
      loopAcc(list.tail, alltweets.filter(x => x.text.contains(list.head)) union acc)
    }
  }

  lazy val googleTweets: TweetSet = loopAcc(google, new Empty)
  lazy val appleTweets: TweetSet = loopAcc(apple, new Empty)
